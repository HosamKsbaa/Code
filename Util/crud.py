...
def getLastMessage(db: Session):
    last_message = db.query(models2.Messages).order_by(
        models2.Messages.MessageId.desc()).first()
    return last_message
...
def create_Message(db: Session, Message: schemas.MessageCreate):
    ...
    send_message(json.dumps(db_message, cls=MessagesEncoder),  Topic= "my-topic2" )
...
def create_Analyze2(db: Session, Analytics: schemas.AnalyticCreate):
    db_Analytics = models2.Analytics(
        neg=Analytics.neg, neu=Analytics.neu, pos=Analytics.pos, compound=Analytics.compound)
    db.add(db_Analytics)
    db.commit()
    db.refresh(db_Analytics)
    # send_message(message)
